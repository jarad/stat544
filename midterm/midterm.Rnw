...
\item Assuming the delivery man picks jar types randomly and uniformly, fill in Table \ref{tab:} below for the joint distribution of chips and jar types. A (5 pts)
...
&\propto \beta^{a-1} e^{-\beta n\overline{y}}
\end{align*}
where $\overline{y}$ is the sample mean of the $y_i$. This is the kernel of a $Ga(a,n\overline{y})$. 
}
...
&\propto \beta^{a_0+a-1} e^{-\beta [b_0+n\overline{y}]}
\end{align*}
}
...
\item Provide 95\% credible intervals (1 pt each) for 
  ...
  \begin{enumerate}
  \item $\mu$ \longcorrect[\vspace{0.3in}]{(0.5,1.4)}
  \item $\rho_1$, the rate of failures for O-rings for company 1 \longcorrect[\vspace{0.3in}]{(0.5,3.7)}
  \item the {\bf mean failure time} for O-rings from company 1 \longcorrect[\vspace{0.3in}]{(0.27,2)}
  \end{enumerate}
  
\item Describe why the median for $\rho$ for th2nd supplier is so low compared to the others. (1 pt)
...
\longcorrect{
  ...
  Alternatively, the number of O-rings needed have a Poisson distribution with mean $12\rho_s$. Thus the expected cost is $E[12C_s\rho_s]=12C_s E[\rho_s]$ and we can use the estimated posterior mean from the JAGS output for each $\rho_s$. 
}
...
<<exponential_model, tidy=FALSE, message=FALSE>>=
library(rjags)
dat = list(n = nrow(d),
           y = d$failure_time,
           supplier = d$supplier,
