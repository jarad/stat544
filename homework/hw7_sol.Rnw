...
Notice that many of the intervals extend farther to the right. The plot is left as is to ease comparison with the hierarchical model. For reference, the red line in the figure is  4pCi/L, the federal limit above which suggests the use of a radon mitigation sytem.
...
Immediately this pvalue suggests that it would be important to incorporate floor into the model. We will compute a posterior predictive pvalue using data replicates from the posterior, i.e. $p(y^{rep}|y) = \int p(y^{rep}|\theta)p(\theta|y) d\theta$, for our hierarchical model. We will compare our observed pvalue to the pvalue from these replicates. 
...
yrep = rdply(nrow(mu), {
  ...
  data.frame(pvalue=diff(t.test(radon~floor, tmp)$estimate))
})

qplot(pvalue, data=yrep, geom="histogram")+geom_vline(x=obs_test_statistic,col=2)
@
