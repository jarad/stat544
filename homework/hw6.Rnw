...
\item Derive the posterior $p(\beta,\sigma|y)=p(\beta|\sigma^2,y)p(\sigma^2|y)$. 
  ...
  \item Derive the marginal posteriof for $\sigma^2$, i.e. $p(\sigma^2|y)$.
  \end{enumerate}
  
\item For $\tilde{y} \sim N(\tilde{X}\beta,\sigma^2\mathrm{I})$ where the $d\times k$ matrix $\tilde{X}$ is known, derive the posterior predictive distribution $p(\tilde{y}|y)$.
  \begin{enumerate}
  \item Derive the conditional posterior predictive distribution $p(\tilde{y}|\sigma^2,y)$.
  \item Use the result from 2a, to derive the posterior predictive distribution $p(\tilde{y}|y)$.
  \end{enumerate}
