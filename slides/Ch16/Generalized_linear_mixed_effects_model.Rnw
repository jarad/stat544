...
\begin{comment}
\begin{frame}[fragile]
<<standard_deviations, dependson="run_mcmc", echo=FALSE, cache=TRUE>>=
...
&\approx \frac{1}{T} \sum_{t=1}^T e^{\rho_p^t+\alpha_d^t}
\end{array} \]
\pause New sow (for $1^{st}$ progeny and average grandsire):
\[ \begin{array}{rl}
E[\tilde{y}_{new}|y] &= \approx \frac{1}{T} \sum_{t=1}^T e^{\rho_1^t+\alpha_{new}^t}
\end{array} \]
where $\alpha_{new}^t \sim N(0,\sigma_{\alpha}^2)$.
}
}
...
\begin{enumerate}
\item Simulate $\mu^t_{d} = e^{\rho_p^t+\alpha_d^t}$. \pause 
\item Simulate $\mu^t_{new} = e^{\rho_p^t+\alpha_{new}^t}$ where $\alpha_{new}^t \sim N\left(0,\sigma^{2^t}_\alpha\right)$. \pause 
\item Calculate $\delta^t = \mu_{new}^t - \mu_d^t$.
\end{enumerate}

\vspace{0.2in} \pause

So $\delta^t$ is a realization expected difference in the number of progeny between a new sow and current sow $d$.
}



\begin{frame}[fragile]
<<zero_nba, dependson="run_mcmc">>=
...
exp_nba_old1 = exp(rhos[,2]+alphas[,wh[1]])
...
hist(exp_nba_old1-exp_nba_new, 100, main="Expected difference (Old-New)")
@
\end{frame}
...
\begin{frame}[fragile]
<<parity_four, dependson="run_mcmc", fig.width=8>>=
...
\begin{frame}[fragile]
\frametitle{Difference in the number of progeny}
...
\[ \tilde{y}_d^t \sim Po(e^{\rho_p^t+\alpha_d^t}) \]
\item New sow (average grand-sire):
\[ \tilde{y}_{new}^t \sim Po(e^{\rho_1^t+\alpha_{new}^t}) \]
\end{itemize}
where $e^{\rho_p^t+\alpha_d^t}$ and $e^{\rho_1^t+\alpha_{new}^t}$ are simulations previously drawn.
\end{frame}
