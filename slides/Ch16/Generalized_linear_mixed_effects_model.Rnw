...
model{
  ...
  # Random effects
  for (d in 1:nd) { alpha[d]  ~ dnorm(0, 1/sigma.alpha^2) }
  for (s in 1:ns) { beta[s]   ~ dnorm(0, 1/sigma.beta^2) }
...
res = coda.samples(m, c("rho","alpha","beta","sigma.alpha", "sigma.beta"), n.iter=100000, thin=100)
@
\end{frame}
