...
p = ncol(X)
out = mcmc(10000, birthwt$low, X, rep(0,p), 3*diag(p))
...
\pause A common assumption is that the random effects are $u\sim N(0,\sigma^2 \I)$. \pause And the prior $\sigma\sim Unif(0,10)$ can be specified. \pause Thus the prior on $\tilde{\beta}$ is 
\[ \tilde{\beta} = 
...
p = ncol(X)
out <- mcmc(10000, dd$y, X, Z, rep(0,p), 10*diag(p))
colnames(out$beta) = colnames(X)
burnin = 1:100
@
\end{frame}
