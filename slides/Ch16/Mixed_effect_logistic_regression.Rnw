...
=& \prod_{i=1}^n N(Z_i|X_i'\beta,1)\left[\I(Z_i>0)\I(y_i=1)+\I(Z_i\le 0)\I(y_i=0)\right] \pause \\
&\times N(\beta|\beta_0,\mySigma_\beta) \pause \\
...
\pause and $u\sim N(0,\sigma^2 \I)$ and $\sigma\sim Unif(0,10)$. \pause Thus the prior on $\tilde{\beta}$ is 
\[ \tilde{\beta} = 
...
&\propto \prod_{u=1}^U N(u_i|0,\sigma^2) \frac{1}{\sigma}\I(0,100) \pause \\
&\propto (\sigma^2)^{-U/2} e^{-\frac{1}{2\sigma^2}u'u} (\sigma^2)^{-1/2}\I(0,100)  \pause \\
...
p = ncol(X)
out <- mcmc(1000, dd$y, X, Z, rep(0,p), 10*diag(p))
colnames(out$beta) = colnames(X)
burnin = 1:100
@
\end{frame}
...
par(mfrow=c(2,3))
for (i in 1:p) acf(out$beta[-burnin,i], main=colnames(X)[i])
@
\end{frame}
...
require(coda)
require(mcmcse)
adply(out$beta[-burnin,], 2, function(x) {
  data.frame(ess_coda = round(effectiveSize(x)),
             ess_mcmcse = round(ess(x)),
             lb = round(quantile(x, .025),2),
             ub = round(quantile(x, .975),2))
...
\section{Additional uses for data augmentation}
\begin{frame}[fragile]
