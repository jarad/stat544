...
\begin{enumerate}
\item propose $\theta^*\sim J(\theta|\theta^t)$
\item accept $\theta^{t+1}=\theta^*$ with probability $\min\{1,r\}$ where 
\[ r = r(\theta^t,\theta^*) = \pause 
\frac{q(\theta^*|y)/J(\theta^*|\theta^t)}{q(\theta^t|y)/J(\theta^t|\theta^*)} = \frac{q(\theta^*|y)}{q(\theta^t|y)}\frac{J(\theta^t|\theta^*)}{J(\theta^*|\theta^t)} \]
...
\pause and acceptance probability is $\min\{1,r\}$ where 
\[ r = \bordermatrix{ & 0 & 1 \cr 0 & 1 & \frac{1/3}{2/3} \frac{0.3}{0.5} \cr 1 & \frac{2/3}{1/3} \frac{0.5}{0.3} & 1 } = \bordermatrix{ & 0 & 1 \cr 0 & 1 & 0.3 \cr 1 & 3.33 & 1 }  \]
Thus, the transition distribution is 
\[\begin{array}{rlll}
p &= p(\theta^{t+1}=1|\theta^t=0) = p(\theta^*=1|\theta^t=0)p(\theta^*\mbox{ accepted}) &= 0.5\times 0.3 &= 0.15 \\
q &= p(\theta^{t+1}=0|\theta^t=1) = p(\theta^*=0|\theta^t=1)p(\theta^*\mbox{ accepted}) &= 0.3\times 1 &= 0.3
\end{array} \]
and we know the station distribution is $(0.3,0.15)/(0.3+0.15)=(2/3\,\,1/3)$. 
\end{frame}
...
\pause Use $N(y,1)$ as the proposal, \pause then the Metropolis-Hastings acceptance probability is the $\min\{1,r\}$ with 
\[ r = \frac{q(\theta^*|y)}{q(\theta^t|y)}\frac{J(\theta^t|\theta^*)}{J(\theta^*|\theta^t)} = \frac{1+(\theta^t)^2}{1+(\theta^*)^2} \] 
\end{frame}
...
\end{itemize}
Independence Metropolis-Hastings also requires heavy tailed proposals since if $\theta^t$ is 
\begin{itemize}
\item in a region where $q(\theta^t|y)>>J(\theta^t)$ then
\item any proposal $\theta^*$ such that $p(\theta^*)\approx J(\theta^*)$ 
\end{itemize}
will result in 
\[ r = \frac{J(\theta^t)}{q(\theta^t|y)}\frac{q(\theta^*|y)}{J(\theta^*)} \approx 0 \]
...
\item propose $\theta^*\sim J(\theta|\theta^t)$
\item accept $\theta^{t+1}=\theta^*$ with probability $\min\{1,r\}$ where 
\[ r = \frac{q(\theta^*|y)}{q(\theta^t|y)}\frac{J(\theta^t|\theta^*)}{J(\theta^*|\theta^t)} =
...
Let $Y \sim Bin(n,\theta)$ and $\theta \sim Be(1/2,1/2)$\pause, thus the posterior is \pause $p(\theta|y)\propto \theta^{y-0.5}(1-\theta)^{n-y-0.5}\mathrm{I}(0<\theta<1)$. 

\vspace{0.2in} \pause

To construct a random-walk Metropolis algorithm, we choose the proposal 
\[ \theta \sim N(\theta^t,\sigma^2) \]
