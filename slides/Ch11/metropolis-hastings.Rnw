...
\pause and acceptance probability
\[ r = \bordermatrix{ & 0 & 1 \cr 0 & 1 & 0.5\times \frac{0.3}{0.5} \cr 1 & 2\times \frac{0.5}{0.3} & 1 } = \]


Then the transition distribution is
\[ \begin{array}{rl} 
p(\theta^{t+1}=1|\theta^t=0) &= J(\theta^*=1|\theta^t=0)*p(\theta^*=1 \mbox{ accepted}) = 0.5 \times 0.3 \\
p(\theta^{t+1}=1|\theta^t=1) &= J(\theta^*=1|\theta^t=1)*p(\theta^*=1 \mbox{ accepted}) = 0.7 
\end{array} \]
...
\pause then 
\[ \begin{array}{rl}
p(\theta^{t-1}=\theta_a, \theta^t=\theta_b) &= p(\theta_a|y)J(\theta_b|\theta_a) \pause \\
p(\theta^{t}=\theta_a, \theta^{t-1}=\theta_b) &= p(\theta_b|y)J(\theta_a|\theta_b) \frac{p(\theta_a|y) J(\theta_b|\theta_a)}{p(\theta_b|y) J(\theta_a|\theta_b)} \pause \\
&= p(\theta_a|y) J(\theta_b|\theta_a)
\end{array} \]
\pause Since the joint distribution is symmetric, the marginal distributions for $\theta^{t-1}$ and $\theta^t$ are the same and equal to $p(\theta|y)$. 
\end{proof}
...
\[ r = \frac{q(\theta^*|y)}{q(\theta^t|y)}\frac{J(\theta^t)}{J(\theta^*)} = \frac{1+(\theta^t)^2}{1+(\theta^*)^2} \] 
\end{frame}
...
\item propose $\theta^*\sim J(\theta)$
\item accept $\theta^{t+1}=\theta^*$ with probability $\min\{1,r\}$ where 
\[ r = \frac{q(\theta^*|y)}{q(\theta^t|y)}\frac{J(\theta^t)}{J(\theta^*)} =
