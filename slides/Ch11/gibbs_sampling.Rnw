...
\frametitle{}
\begin{theorem}
If $K(\theta,\theta')= p(\theta_2'|\theta_1',y)p(\theta_1'|\theta_2,y)$ then $p(\theta)$ is the stationary distribution, i.e.
\[ p(\theta'|y) = \int K(\theta,\theta') p(\theta|y) d\theta. \]
\end{theorem}

\begin{proof}
\[ \begin{array}{rl}
p(\theta'|y) &= p(\theta_2'|\theta_1',y)p(\theta_1'|y) \\
&= p(\theta_2'|\theta_1',y)\int p(\theta_1'|\theta_2,y) p(\theta_2|y) d\theta_2 \\
&= p(\theta_2'|\theta_1',y)\int p(\theta_1'|\theta_2,y) \int p(\theta_2|\theta_1,y)p(\theta_1|y) d\theta_1 d\theta_2 \\
&= \int \int p(\theta_2'|\theta_1',y) p(\theta_1'|\theta_2,y) \int p(\theta_2|\theta_1,y)p(\theta_1|y) d\theta_1 d\theta_2 \\
...
\pause \alert{Slice sampling} performs the following Gibbs sampler: \pause 
\begin{enumerate}
\item $u^t|\theta^{t-1} \sim \mbox{Unif}(0<u<p(\theta^{t-1}|y)$ \pause and
\item $\theta^t|u^t \sim \mbox{Unif}\{\theta: u<p(\theta|y) \}$.
\end{enumerate}
This is a \alert{latent variable augmentation}.
\end{frame}
