...
\begin{frame}[fragile]
<<mcmc>>=
# use x=theta for alpha and x=1-theta for beta
log_fc = function(x, alpha, beta) {
  if (alpha<0 | beta<0) return(-Inf)
  n = length(x)
  alpha*sum(log(x))-n*lbeta(alpha,beta)-5/2*(alpha+beta)
}
mcmc = function(n_sims, dat, inits, tune) {
  ...
  for (i in 1:n_sims) {
    ...
    logr = log_fc(theta, alpha_prop, beta)-log_fc(theta, alpha, beta)
    alpha = ifelse(log(runif(1))<logr, alpha_prop, alpha)
    
    # Sample beta
    beta_prop = rnorm(1, beta, tune$beta)
    logr = log_fc(1-theta, alpha, beta_prop)-log_fc(1-theta, alpha, beta)
...
r = mcmc(100, dat=dat, inits=inits, tune=list(alpha=1,beta=1))
@
\end{frame}
