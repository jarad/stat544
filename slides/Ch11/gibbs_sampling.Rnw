...
\frame{\frametitle{Gibbs sampling}
  Suppose $\theta_1,\theta_2\sim p\left(\theta_1,\theta_2\right)$ \pause and we can sample from $p\left(\theta_1|\theta_2\right)$ and $p\left(\theta_2|\theta_1\right)$\pause. Beginning with an initial value $\left(\theta_1^{(0)}, \theta_2^{(0)}\right)$\pause, the Gibbs sampler is 
	\begin{enumerate}[\quad1.]
	\item sample $\theta_1^{(j)}\sim p\left(\theta_1|\theta_2^{(j-1)}\right)$ \pause and then 
	\item sample $\theta_2^{(j)}\sim p\left(\theta_2|\theta_1^{(j)}\right)$. 
	\end{enumerate}
	...
	Notes:
	\begin{itemize}
	\item $\theta^{(j)} \stackrel{d}{\rightarrow} \theta$ where $\theta = (\theta_1,\theta_2)' \sim p\left(\theta_1,\theta_2\right)$.  \pause
	\item The sequence $\theta^{(j)}$ is not independent. \pause
	\item 
	\[  \frac{1}{J} \sum_{j=1}^J h\left(\theta^{(j)}\right) \rightarrow E_p[h(\theta)] = \int_{\Theta} h(\theta) f(\theta) d\theta  \]
	\end{itemize}
}
...
\frame{\frametitle{Bivariate normal example}
	...
	\pause Then 
	\[ \begin{array}{rl}
	\theta_1|\theta_2 &\sim N\left(\rho\theta_2, [1-\rho^2]\right) \pause \\
...
\frame{\frametitle{$K$-component Gibbs sampler}
	...
	\[ \begin{array}{rl}
	\theta_1^{(j)} &\sim p\left(\theta_1|\theta_2^{(j-1)},\ldots, \theta_K^{(j-1)}\right) \pause \\ \\
	\theta_2^{(j)} &\sim p\left(\theta_2|\theta_1^{(j)}, \theta_3^{(j-1)},\ldots, \theta_K^{(j-1)}\right) \pause\\
	&\vdots \\
	\theta_k^{(j)} &\sim p\left(\theta_k|\theta_1^{(j)}\ldots,\theta_{k-1}^{(j)}, \theta_{k+1}^{(j-1)},\ldots, \theta_K^{(j-1)}\right) \\
	&\vdots \pause\\
	\theta_K^{(j)} &\sim p\left(\theta_K|\theta_1^{(j)},\ldots, \theta_{K-1}^{(j)}\right) \pause
	\end{array} \]
...
\frame{\frametitle{Metropolis-within-Gibbs}
  Previously we have discussed two Markov chain approaches to sampling from target distributions:
	\begin{itemize}
	...
	Gibbs sampling assumed we can sample from $p(\theta_k|\theta_{-k})$ for all $k$\pause, but what if cannot sample from all of these full conditional distributions? \pause For those $p(\theta_k|\theta_{-k})$ that cannot be sampled directly, a Metropolis-Hastings step can be substituted. \pause All the convergence results for Gibbs sampling and Metropolis-Hastings algorithm still hold. 
}

\subsection{Poisson hierarchical model example}
\frame{\frametitle{Poisson hierarchical model example}
	Consider the bladder cancer data set previously discussed, namely
	\begin{itemize}
	\item $y_k$ be the number of observed deaths in county $k$ \pause 
	\item $n_k$ be the number of person-years in county $k$ divided by 100,000 \pause
	\item $\theta_k$ is the expected deaths per 100,000 person-years
	\end{itemize}
	...
	Assume 
	\[ \begin{array}{rl}
	y_k &\stackrel{ind}{\sim} Po(n_k\theta_k) \pause \\
	\theta_k &\stackrel{iid}{\sim} Ga(\alpha,\beta) \pause \\
	\alpha &\sim Unif(0,a_0) \\
	\beta &\sim Unif(0,b_0) \pause
	\end{array} \]
...
\footnotesize 
	The target distribution here is 
	\[ 
	\begin{array}{rl}
	p(\theta,\alpha,\beta|y) &\pause \propto \prod_{k=1}^K \left[p(y_k|\theta_k,n_k) p(\theta_k|\alpha,\beta)\right] p(\alpha)p(\beta) \pause \\
	& \propto \prod_{k=1}^K \left[e^{-n_k\theta_k}\theta_k^{y_k} \frac{\beta^\alpha}{\mathrm{\Gamma}(\alpha)} \theta_k^{\alpha-1}e^{-\theta_k \beta} \right] \mathrm{I}(0<\alpha<a_0) \mathrm{I}(0<\beta<b_0) \\
	\end{array} 
	\]
	\pause The full conditionals are 
	\[ \begin{array}{rl}
	p(\theta_k|y,\theta_{-k},\alpha,\beta) &\propto p(\theta,\alpha,\beta|y) \pause \\
	&\propto e^{-n_k\theta_k}\theta_k^{y_k}  \theta_k^{\alpha-1}e^{-\theta_k \beta} \pause \\
	&= \theta_k^{y_k+\alpha-1}e^{-\theta_k(n_k+\beta)} \pause \\
	&\propto Ga(y_k+\alpha, n_k+\beta) \pause \\
	\\
	p(\alpha|y,\theta,\beta) &\propto p(\theta,\alpha,\beta|y) \pause \\
	&\propto \prod_{k=1}^K \left[\theta_k^{\alpha-1} \frac{\beta^\alpha}{\mathrm{\Gamma}(\alpha)} \right] \mathrm{I}(0<\alpha<a_0) \pause \\
	&= \left( \prod_{k=1}^K \theta_k \right)^{\alpha} \beta^{K\alpha} \mathrm{\Gamma}(\alpha)^{-K} \mathrm{I}(0<\alpha<a_0) \pause \\
	\\
	p(\beta|y,\theta,\alpha) &\propto p(\theta,\alpha,\beta|y) \pause \\
	&\propto \prod_{k=1}^K \left[e^{-\theta_k \beta} \beta^\alpha \right] \mathrm{I}(0<\beta<b_0) \pause \\
	&= \beta^{K\alpha} e^{- \beta\sum_{k=1}^K \theta_k} \mathrm{I}(0<\beta<b_0) \pause \\
	&\propto Ga(K\alpha+1, \sum_{k=1}^K \theta_k) \mathrm{I}(0<\beta<b_0) 
	\end{array} \]
}
...
\frame{\frametitle{Random-walk Metropolis for $\alpha$}
	...
	So we will use a random-walk Metropolis algorithm with $N(\alpha^{(j)},\sigma^2)$ as our proposal distribution. \pause The acceptance probability is the minimum of 1 and 
	\[ \frac{p(\alpha^*|y,\theta,\beta)}{p(\alpha^{(j)}|y,\theta,\beta)} = \left( \prod_{k=1}^K \theta_k \right)^{\alpha^*-\alpha^{(j)}} \beta^{K(\alpha^*-\alpha^{(j)})} \left( \frac{\mathrm{\Gamma}(\alpha^*)}{\mathrm{\Gamma}(\alpha^{(j)})} \right)^{-K} \mathrm{I}(0<\alpha^*<a_0) \]
	Notice that $0< \alpha^{(j)} < a_0$ or else the result is undefined. \pause This means that $\alpha^{(0)}$ must be between 0 and $a_0$. \pause $\sigma^2$ is a tuning parameter
}

\frame{\frametitle{Tuning a random-walk}
\small
	Random-walk proposals typically have a \alert{tuning parameter}, e.g. $\sigma^2$ in $N(\alpha^{(j)},\sigma^2)$, which must be set. \pause 
	\begin{center}
...
\frame{\frametitle{Markov chain Monte Carlo}
	...
	\begin{enumerate}[\quad1.]
	\item Sample $\theta^{(j+1)}\sim p\left(\theta|y,\alpha^{(j)},\beta^{(j)}\right)$: \pause 
		\begin{itemize}
		\item For $k=1,\ldots,K$, sample $\theta_k^{(j+1)} \sim p\left(\theta_k|y_k,\alpha^{(j)},\beta^{(j)}\right)$. \pause
		\end{itemize} 
	\item Sample $\alpha^{(j+1)} \sim p\left(\alpha|\theta^{(j+1)},\beta^{(j)}\right)$ using random walk Metropolis. \pause 
	\item Sample $\beta^{(j+1)} \sim p\left(\beta|\theta^{(j+1)},\alpha^{(j+1)}\right)$ using the inverse CDF method. \pause
	\end{enumerate}
	is a \alert{Markov chain Monte Carlo} algorithm \pause whose stationary distribution, by construction, is $p(\theta,\alpha,\beta|y)$. \pause So, we know $\left(\theta^{(j)},\alpha^{(j)},\beta^{(j)}\right)$ converges to a draw from $p(\theta,\alpha,\beta|y)$.
}
