...
\frame{\frametitle{Model construction for BUGS/JAGS/Stan}
  An alternative construction of the model convenient for programming in BUGS/JAGS/Stan \pause uses the notation
	\begin{itemize}
...
\frame{\frametitle{Bayesian model}
  ...
  \begin{itemize}
	...
	Prior: $p(\sigma_e,\sigma_u) \propto \I(0<\sigma_e<100) \I(0<\sigma_u<100)$.
}
...
<<jags_model, tidy=FALSE, cache=TRUE>>=
jags_model = "
model {
  for (i in 1:n) {
    y[i] ~ dnorm(gamma[genotype[i]]+tau[tray[i]], 1/sigma_e^2)
  }
  
  # Fixed effects
  for (g in 1:n_genotypes) { 
    gamma[g]   ~ dnorm(0, 1e-6) # Approximation to uniform
  } 

  # Random effects
  for (t in 1:n_trays) { 
    tau[t]   ~ dnorm(0, 1/sigma_tau^2)
  }

  sigma_e   ~ dunif(0,100)
  sigma_tau ~ dunif(0,100)
...
<<jags_mcmc, tidy=FALSE, cache=TRUE, dependson="jags_model">>=
...
m_jags = jags.model(textConnection(jags_model), dat, n.chains=4)
r_jags = coda.samples(m_jags, c("gamma","tau","sigma_e","sigma_tau","diff"), n.iter=10000, thin=10)
@
...
<<jags_summary, tidy=FALSE, cache=TRUE, dependson="jags_mcmc">>=
summary(r_jags)
@
...
\subsection{Results}
\begin{frame}[fragile]
<<analysis, dependson="stan_model", cache=TRUE, message=FALSE>>=
m_stan = stan_model(model_code=stan_model)
r_stan = sampling(m_stan, dat, c("gamma","tau","sigma_e","sigma_tau","diff"))
@
\end{frame}
...
<<comparison>>=
plot(density(unlist(r_jags[,"diff"])), main="Posterior for the genotype difference in mean seedling weight",
     xlab="Difference",ylab="Posterior density estimate")
lines(density(extract(r_stan,"diff")$diff), col="red")
@
...
\subsection{Summary}
\frame{\frametitle{Variety of ways to specify hierarchical linear models}
  General mixed-effect models
	...
	The particular example in this lecture:
	\begin{itemize}
