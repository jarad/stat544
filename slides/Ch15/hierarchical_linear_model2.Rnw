...
sm_f = ddply(f, .(abbrev), summarize, min_count=min(count))
f = filter(f, abbrev %in% sm_f$abbrev[sm_f$min_count>0])
f$abbrev = factor(f$abbrev)
...
<<iw_compile, eval=FALSE>>=
m_iw = stan_model(model_code = inverse_wishart_model)
@

<<iw_run, cache=TRUE, eval=FALSE>>=
r = sampling(m_iw,
...
model {
  for (i in 1:n_species) {
    y[i] ~ dnorm(beta[species[i],1]+beta[species[i],2]*year[i], 1/sigma^2)
    beta[i,1:2] ~ dmnorm(beta0, Omega_beta) 
  }

  sigma ~ dunif(0,100)

  beta0[1] ~ dnorm(0,1e-6)
  beta0[2] ~ dnorm(0,1e-6)

  Omega_beta ~ dwish(eye,3)

  Sigma_beta <- inverse(Omega_beta)
...
dat = list(n_species=nlevels(f$abbrev),
           y = f$ave,
           species = as.numeric(f$abbrev),
           year = f$year-1995,
           eye = diag(nrow=2))

m = jags.model(textConnection(iw_jags), dat)

r= coda.samples(m, c("beta","rho","sigma","sigma1","sigma2"),10000)


plot(r[,c("rho","sigma","sigma1","sigma2")], ask=TRUE)
...
model {
  ...
  Sigma_beta[1,1] <- sigma1*sigma1;
  Sigma_beta[2,2] <- sigma2*sigma2;
  Sigma_beta[1,2] <- rho*sigma1*sigma2;
  Sigma_beta[2,1] <- rho*sigma1*sigma2;
...
m_ss = jags.model(textConnection(ss_jags), dat)

r_ss = coda.samples(m_ss, c("beta","rho","sigma","sigma1","sigma2"),100000,thin=10)


plot(r_ss[,c("rho","sigma","sigma1","sigma2")], ask=TRUE)
