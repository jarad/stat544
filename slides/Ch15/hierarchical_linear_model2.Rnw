...
\frametitle{Random intercept, random slope model} 
  ...
  \end{array} \]
  where $\beta_j=(\beta_{0j},\beta_{1j})^\top$. 
...
Special cases:
\begin{itemize}
\item If $\nu=d+1$, then each of the correlations in $\mySigma$ has a marginal uniform prior. 
...
or, equivalently, 
\[ \sigma_y^{-2}|\ldots \sim Ga(v/2,nv/2). \]
...
{\small
Assume $\mySigma_\beta \sim IW(d+1,\mathrm{I})$, then 
\[ \begin{array}{rl}
...
&\times |\mySigma|^{-\frac{d+1+d+1}{2}}e^{-\frac{1}{2} \mbox{tr}\left(\mySigma_\beta^{-1}\right)} \\
\propto& |\mySigma|^{-\frac{d+1+J+d+1}{2}} e^{-\frac{1}{2} \mbox{tr}\left(S_n\mySigma_\beta^{-1}\right)} 
\end{array} \]
where 
...
so 
\[ \mySigma_\beta|\ldots \sim IW(d+1+J, S_n) \]
or, equivalently, 
...
\begin{enumerate}[<+->]
\item $p(\beta|\ldots)$: regression with informative prior
\item $p(\sigma_y^2|\ldots)$: normal with known mean
\item $p(\mu_\beta|\ldots)$: multivariate normal with known covariance matrix
\item $p(\mySigma_\beta|\ldots)$: multivariat normal with known mean
\end{enumerate}
\end{frame}
...
\propto& \prod_{j=1}^J e^{-(\beta_j-\mu_\beta)^\top \mySigma_\beta^{-1}(\beta_j-\mu_\beta)/2} \\
\propto& e^{-[\mu_\beta' (J \mySigma_{\beta}^{-1})\mu_\beta-2\mu_\beta \mySigma_\beta^{-1} \sum_{j=1}^J \beta_j  ]/2}
\end{array} \]
Thus
\[ \mu_\beta|\ldots ~ N(\hat{\mu}_\beta, \hat{\mySigma}_\mu) \]
with 
\[ \begin{array}{rl}
\hat{\mySigma}_\mu &= \mySigma_\beta/J \\
...
\begin{frame}[fragile]
<<run_mcmc, cache=TRUE>>=
out = mcmc(1000, y, cbind(1,x), group, 3, diag(2))
@
\end{frame}
