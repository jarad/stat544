...
\frame{\frametitle{Single-parameter models (Ch 2, L1)}
  \begin{multicols}{2}
  General
  \begin{itemize}
  \item Priors (L2)
  ...
  \item Normal, unknown mean (Sec 2.5, L3)
  \item Normal, unknown variance (Sec 2.6, L3)
...
\frame{\frametitle{Single-parameter models (Ch 2, L1)}
  ...
  \item Point estimation  depends on utility function
    ...
    \end{itemize}
  \item Computation, $E[h(\theta)|y]$ (L4)
    \begin{itemize}
    \item SLLN
    \item CLT
    \end{itemize}
...
\frame{\frametitle{Introduction to multiparameter models (Ch 3, L5)}
  \begin{itemize}
...
=======
  ...
  \pause results in 
>>>>>>> 1dadbb500ef5e79cbf68db9dee94c20c04b50651
...
\frame{\frametitle{Hypothesis testing (L6)}
  From a Bayesian perspective, 
  \begin{itemize}[<+->]
  \item treat point hypotheses as formal Bayesian tests
  \[ H_i: \theta=\theta_i \]
  \item treat composite hypotheses via parameter estimation 
  \[ H_i: \theta \in (\theta_i,\theta_{i+1}] \]
  \end{itemize}
  
  \vspace{0.2in} \pause
  
  Formal Bayesian hypotheses 
  \begin{itemize}[<+->]
...
\frame{\frametitle{Hierarchical models (Ch 5, L7, L12)}
  \begin{itemize}[<+->]
  ...
  \item Hierarchical normal model (Sec 5.4, HW 7, L8)
...
\frame{\frametitle{Introduction to regression models (Ch 14, L9, L10)}
  \begin{itemize}
...
=======
  \pause where $\hat{\beta} = [X^\top X]^{-1} X^top y$ and $s^2 = \frac{1}{n-k} (y-X\hat{\beta})^\top (y-X\hat{\beta})$. \pause 
>>>>>>> 1dadbb500ef5e79cbf68db9dee94c20c04b50651
...
<<<<<<< HEAD
  ...
  can be thought of as additional data directly on $\beta$ and $\sigma^2$.
=======
...
\frame{\frametitle{Model checking (Ch 6, L11, HW7)}
  \begin{itemize}[<+->]
