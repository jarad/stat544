...
mle = ddply(data.frame(tau=2:(nrow(d)-1)), .(tau), function(x) {
  ...
  data.frame(mu0 = tmp$mean[1], mu1 = tmp$mean[2], 
...
How would you find a confidence interval/set? 
...
m = jags.model(textConnection(model), 
               data=list(n=nrow(d), y=d$texts, p=rep(1/nrow(d), nrow(d))))
r = coda.samples(m, c("mu","tau"), 10000, thin=10)
@
\end{frame}
