...
Suppose $p(\theta|y)$ is unimodal and roughly symmetric\pause, then a Taylor series expansion of the logarithm of the posterior around the posterior mode $\hat{\theta}$ is 
...
Let $y \sim Bin(n,\theta)$ and $\theta \sim Be(\alpha,\beta)$, then $\theta|y\sim \pause Be(\alpha+y,\beta+n-y)$ and the posterior mode is 
...
\begin{theorem}
If the parameter space $\Theta$ is finite and $Pr(\theta=\theta_0)>0$, then $P(\theta=\theta_0|y)\to 1$ as $n\to\infty$. 
\end{theorem}

\begin{theorem}
If the parameter space $\Theta$ is continuous and $A$ is a neighborhood around $\theta_0$ with and $Pr(\theta\in A)>0$, then $P(\theta\in A|y)\to 1$ as $n\to\infty$. 
\end{theorem}
...
{\tiny 
\[ \log p(\theta|y) = \log p(\hat{\theta}|y) + \frac{1}{2} (\theta-\hat{\theta})^2 \frac{d^2}{d\theta^2} [\log p(\theta|y)]_{\theta=\hat{\theta}} + \frac{1}{6} (\theta-\hat{\theta})^3 [\log p(\theta|y)]_{\theta=\hat{\theta}} + \cdots \]
}

where the linear term is zero because the derivative at the posterior mode $\hat{\theta}$ is zero. 

\vspace{0.2in} \pause

The coefficient for the quadratic term can be written as 
\[ \frac{d^2}{d\theta^2} [\log p(\theta|y)]_{\theta=\hat{\theta}} = \frac{d^2}{d\theta^2} \log p(\hat{\theta}) + \sum_{i=1}^n \frac{d^2}{d\theta^2} [\log p(y_i|\theta)]_{\theta=\hat{\theta}}\]
where 
\[ E\left[ \frac{d^2}{d\theta^2} [\log p(y_i|\theta)]_{\theta=\hat{\theta}}\right] = -\J(\theta_0) \]
where $\J(\theta_0)$ is the expected Fisher information and thus, by the LLN, the second term converges to $-n\J(\theta_0)$. 
}
\end{frame}
