...
\frac{d^2}{d\theta^2} \log p(\hat{\theta}) + 
...
\log p(\theta|y) \approx
\log p(\hat{\theta}|y) + 
\frac{1}{2} (\theta-\hat{\theta})^2 \left[ -n\J(\theta_0) \right]  \]
and since $\hat{\theta}\to\theta_0$, we have 
\[ p(\theta|y) \propto \exp\left(-\frac{1}{2 \frac{1}{n} \J(\hat{\theta})^{-1}} (\theta-\hat{\theta})^2 \right) \]
