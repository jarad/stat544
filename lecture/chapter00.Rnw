...
If we are interested in the hypotheses $H_0: \theta=0.5$ vs $H_1: \theta<0.5$, then the pvalue for the experiments are 0.03 and 0.07. 
And if we stick with the standard 0.05 cutoff, we would make different decisions about $\theta$ even though are data are exactly the same, i.e. 3 heads and 9 tails.  
\end{example}
