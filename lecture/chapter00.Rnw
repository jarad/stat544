...
The disadvantages are computation, specifying a prior, and Bayesian not the standard.
In the remainder of this section, we take a brief look at each of these advantages and disadvantages, so that readers will be consciously thinking about them as they read the remainder of the text. 
...
We will see that Bayesian analyzes typically require much more computation time than the associated non-Bayesian analyzes. 
...
\paragraph{Specifying a prior}

Science strives toward a notion of \emph{objectivity}. 
Objectivity means that, aside from randomness in the world, two scientists who conduct the same experiment should arrive at the same conclusions. 
This means that there should not be bias in the way the experiment is conducted and there should not be bias in the way the data are analyzed. 
The former is best accomplished by blinding the researchers and the latter can be accomplished by researchers having standard statistical tools for analysis.

These statistical tools require the specification of a statistical model which is generally made up but if researchers can agree to it, then we can have comparable results.
Bayesians have the added complication of specification of a prior which is also generally made up, but this additional assumption appears to have more disagreement than models have. 
...
It is commonly stated that ``Bayesians treat parameters as random while non-Bayesians treat parameters as fixed.'' While this is true, most Bayesian believe that the value of a parameter in a particular model has a fixed, but unknown. Since it is unknown, Bayesians assign it a probability distribution and thereby \emph{treat it as random} just as we treat data prior to its collection as random. 

A better quote may be ``Bayesians treat parameters as unknown while frequentists treat parameters as known.'' 
While perhaps a bit inflammatory for frequentists, it makes reference to the fact that pvalues are constructed based on the null hypothesis being true, confidence intervals are often constructed by plugging the maximum likelihood estimate into the Hessian, and prediction is often based on plugging in point estimates for some parameters, notably variances.
